...
export default function MedicationEdit({ medication, dosageForms = [], suppliers = [], activeIngredients = [] }) {
    const { data, setData, put, processing, recentlySuccessful, errors } = useForm({
        name: medication.name || '',
        dosage_form_id: medication.dosage_form_id || '',
        schedule: medication.schedule || '',
        current_sale_price: medication.current_sale_price || '',
        supplier_id: medication.supplier_id || '',
        reorder_level: medication.reorder_level || '',
        quantity_on_hand: medication.quantity_on_hand || '',
        active_ingredients: medication.active_ingredients?.length > 0 ? medication.active_ingredients : [{ id: '', strength: '' }],
    });
    ...
    const removeIngredient = (idx: number) => {
        setData(
            'active_ingredients',
            data.active_ingredients.filter((_, i: number) => i !== idx),
        );
    };
    ...
    return (
        <AppLayout>
            ...
            <div className="mx-auto max-w-2xl px-4 py-6 sm:px-6 lg:px-8">
                <Heading title="Edit Medication" description="Update details for this medication." />
                <Card className="mt-6">
                    <CardHeader>
